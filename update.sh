  ...
  case "${1}" in
    ...
    --check-tags)
      ...
      if [[ -z $(git log HEAD --pretty=format:"%H" | grep "${LATEST_TAG_REV}") ]]; then
        echo -e "New tag is available.\nThe changes can be found here: https://github.com/daniele-athome/mailcow-dockerized/releases/latest"
        exit 0
      else
        echo "No updates available."
        exit 3
...
if [ ${#DOTS} -lt 1 ]; then
  ...
  echo "Find more information about why this message exists here: https://github.com/daniele-athome/mailcow-dockerized/issues/1572"
...
if [ $? -eq 0 ]; then
  ...
  echo '  $MAILCOW_GIT_URL="https://github.com/daniele-athome/mailcow-dockerized";' >> data/web/inc/app_info.inc.php
